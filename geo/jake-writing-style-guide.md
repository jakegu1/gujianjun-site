...
**[2026-09-15] [全局] 七条铁律并入写作规则（来自 Jake 的项目简报，全文见 docs/BRIEF.md §4，违反任何一条整篇重写）**：①零编造——只用素材包和已发布的事实，缺就写「【待补：xxx】」，绝不补一个听起来合理的数字、客户名、时间或职位；②每个数字带口径（时间范围 + 数据来源）；③不利信息照写，写成"我发现的问题和我的应对"；④保护雇主——内部定价、CPL/转化率等商业敏感绝对值、同事信息、API key/账户 ID 不公开，用百分比、相对变化或区间；⑤不写"我负责了/参与了"的简历腔，写"问题→判断→取舍→结果→重来会怎样"；⑥额外禁用「在当今数字化时代」「赋能」「全方位」「深度解析」「总而言之」「不仅……更……」排比和段尾升华；⑦一篇只打一个点。附带两条：假设性例子必须明确写成假设，不能冒充 Jake 的经历；prompt 模板里"没有数据就用合理估计"的旧规则作废。
